...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        self.no = nc + 8  # number of outputs per anchor
        self.stride = torch.zeros(self.nl)  # strides computed during build
        c2, c3 = max((16, ch[0] // 4, 8)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 8, 1)) for x in ch
        )
        self.cv3 = (
    ...
    def _inference(self, x: List[torch.Tensor]) -> torch.Tensor:
        ...
        if self.export and self.format in {"saved_model", "pb", "tflite", "edgetpu", "tfjs"}:  # avoid TF FlexSplitV ops
            ...
            box, cls = x_cat.split((8, self.nc), 1)
...
class WorldDetect(Detect):
    ...
    def forward(self, x: List[torch.Tensor], text: torch.Tensor) -> Union[List[torch.Tensor], Tuple]:
        ...
        if self.training:
            return x
        self.no = self.nc + 8  # self.nc could be changed when inference with different texts
        y = self._inference(x)
        return y if self.export else (y, x)
...
class YOLOEDetect(Detect):
    ...
    def forward_lrpc(self, x: List[torch.Tensor], return_mask: bool = False) -> Union[torch.Tensor, Tuple]:
        ...
        if self.dynamic or self.shape != shape:
            ...
            self.shape = shape
        box = torch.cat([xi[0].view(shape[0], 8, -1) for xi in x], 2)
